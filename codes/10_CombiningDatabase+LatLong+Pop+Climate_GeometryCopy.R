...
# This is to save centroids only 
...
States_AllData_geometry <- States_AllData %>%
  ...
  select(-centroid)   # drop temporary centroid geometry


# Save as a multipolygon shapefile
st_write(States_AllData_geometry, "data/shp/States_AllData_geometry.shp", delete_layer = TRUE)
names(States_AllData_geometry)


# View result
head(States_AllData_geometry)

unique_ids <- unique_ids %>%
  left_join(States_AllData_geometry, by = "UniqueID")
...
villageYears <- unique_ids %>%
  ...
  select(UniqueID, Year, state_code, district_code, Longitude, Latitude, geometry, Shape_Area)
...
st_write(villageYears, "data/shp/VillageYears_ID_geometry.shp", delete_layer = TRUE)
...
village_aqua_salinity_surge_sf <- States_AllData_geometry %>%
  left_join(village_aqua_salinity_surge, by = "UniqueID")

st_write(village_aqua_salinity_surge_sf, "data/shp/village_aqua_salinity_surge.shp", delete_layer = TRUE)
...
aqua_salinity_surge_sf <- States_AllData_geometry %>%
  left_join(aqua_salinity_surge, by = "UniqueID")

st_write(aqua_salinity_surge_sf, "data/shp/aqua_salinity_surge_1990_2025.shp", delete_layer = TRUE)

...
long_landsat8 <- landsat8_results %>%
  select(UniqueID, year, Aqua, DryAqua) %>%
  rename(Year = year)
...
names(combine_landsat5)

# Convert to Longform 
# Pivot Aqua_* and DryAqua_* columns into long format
long_landsat5 <- combine_landsat5 %>%
  pivot_longer(
    cols = starts_with("Aqua_") | starts_with("DryAqua_"),
    names_to = c(".value", "Year"),
    names_pattern = "(Aqua|DryAqua)_(\\d+)"
  ) %>%
...
# Combine landsat 5 with landsat 8 data 
combined_aqua_long <- bind_rows(long_landsat5, long_landsat8)
...
# Print summary
cat("\nData conversion complete. Total records:", nrow(combined_aqua_long), "\n")
cat("Years included:", paste(sort(unique(combined_aqua_long$Year)), collapse=", "), "\n")
cat("Total unique villages:", length(unique(combined_aqua_long$UniqueID)), "\n")


# Save to CSV
write.csv(combined_aqua_long, "outputs/aquaculture_by_year_long_format.csv", row.names = FALSE)
